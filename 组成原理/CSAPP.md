...
- [第一部分-程序结构和执行](#第一部分-程序结构和执行)
  ...
  - [第三章-程序的机器级表示](#第三章-程序的机器级表示)
    ...
    - [3.4 访问信息](#34-访问信息)
      ...
      - [3.4.2 数据传送指令`mov`](#342-数据传送指令mov)
        - [1. 简单数据传送指令`mov[bwlq]`](#1-简单数据传送指令movbwlq)
        - [2. 零扩展和符号扩展数据传送指令`mov[zs]`](#2-零扩展和符号扩展数据传送指令movzs)
      - [3.4.3 数据传送示例](#343-数据传送示例)
      - [3.4.4 压入和弹出栈数据`push`，`pop`](#344-压入和弹出栈数据pushpop)
    - [3.5 算数和逻辑操作](#35-算数和逻辑操作)
      - [3.5.1 加载有效地址`lea`](#351-加载有效地址lea)
...
总共有三类数据传送指令，**每一类指令执行相同的操作，但是操作数大小不同**：

##### 1. 简单数据传送指令`mov[bwlq]`
>选取何种简单数据传送指令的关键点：**寄存器部分**的大小必须与**指令结束的符号**相匹配

Source（存储在寄存器|内存中的一个立即数） --> Destination（寄存器|地址）
规定：S和D不能都指向内存，必须**使用寄存器作为过渡**

如前所述，`movl`在以寄存器为目标的时候会将寄存器的高32位置零

常规的`movq`只能以表示为32位补码的立即数作为源操作数，然后将值符号扩展为64位，放到目标位置。
`moveabsq`可以以任意64位立即数作为源操作数，且只能以寄存器作为目的地

##### 2. 零扩展和符号扩展数据传送指令`mov[zs]`

>这两类数据移动指令的目的都是将较小的源值（**寄存器或内存**）复制到较大的目的（只能是**寄存器**）时使用

区别在于：`movz`零填充高位，`movs`符号填充高位（在传送之前对源操作数做扩展）

`mov[z|s][p][q]`p和q分别表示源操作数和目标操作数的大小

没有`movzlq`这个指令，技术上可以使用`movl`指令代替，因为空余位自动填充0
>`movl`在以寄存器为目标的时候会将寄存器的高32位置零

`cltq`指令没有操作数，只用于`%eax`符号扩展到`%rax`

```assem
movl %eax, %rdx
//如果源和目的都为寄存器，寄存器的大小必须保持一致
```

#### 3.4.3 数据传送示例

1. 参数通过寄存器传递给函数
2. 在函数返回的时候，通过把值存放在`%rax`寄存器中返回数据

>C语言中指针就是地址

函数通过指针传参（**间接引用**），就是**将地址放在寄存器中，随后通过该寄存器找到内存中的地址**
`movq (%rdx), %rax`

>指针的间接引用（pointer dereferencing）通过读取指针所指向的地址，从而获得内存的值

一般来说局部变量存放在寄存器中而不是内存中，寄存器速度比内存快得多

#### 3.4.4 压入和弹出栈数据`push`，`pop`

**压入和弹出的一段被称为栈顶**

1. x86-64的程序栈存放在内存中的某一个区域
2. 栈顶位于低地址，栈底位于高地址，栈向低地址方向增长（好处就是可以用`%rsp`加法来索引栈元素）
3. `%rsp`保存栈顶指针
4. 被弹出的数据依然会被保留在内存中
...
`leaq S, D`的效果被描述为`D =  &S`
**将一个有效地址写入到D**（S是一个地址）

1. `lea`可以用于产生指针，或是进行简单的算术运算（编译器将简单的算数运算，由加法和乘法组成的运算编译成`lea`形式）
2. 目的操作数必须是一个寄存器
3. `lea`指令在执行的过程中不会有访问内存发生

#### 3.5.2 一元和二元操作

一元操作，既是源也是目的，可以是寄存器，也可以是内存位置

对于二元操作`cmd S, D`等价于`D = D cmd S`
S可以是**立即数，寄存器或是内存位置**，第二个数可以是**寄存器或是内存位置**
如果第二个数是内存，处理器将先从内存中取值，执行操作，再写回内存
