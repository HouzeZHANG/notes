  ...
  - [第二章-信息的表示和处理](#第二章-信息的表示和处理)
    - [2.1 信息存储](#21-信息存储)
      - [2.1.1 十六进制表示法](#211-十六进制表示法)
        - [练习2.1-一位十六进制对应四位二进制](#练习21-一位十六进制对应四位二进制)
...
why **十六进制hexadecimal**：因为二进制表示法**过于冗长**，而十进制和二进制的**转换太麻烦**
...
$$x=2^n$$
$$n={i+4j}\;{(i\leq{3})}$$
因为二进制下的n次方意味着数字的结尾有多少个0，因为$0000_{2}=0_{16}$，所以$0xx$结尾有$j$个$0$。对于$i$，如果$i$分别等于$0,1,2,3$意味着以$1_{2}, 10_{2}, 100_{2}, 1000_{2}$开头，对应十六进制$1_{16}, 2_{16}, 4_{16}, 8_{16}$（数电课上讲的8421顺口溜==）
...
1. C语言没有规定编码方式，但大多数实现都使用补码
2. 大多数数字都默认为有符号，要创建无符号数据，使用`123u`或者`123U`
3. 在C中符号数和无符号数之间的强制转换**保持位不变**（上述规则）
