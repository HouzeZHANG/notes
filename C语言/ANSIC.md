...
与之相对应，浮点数相除，无论操作数正负与否，都不会舍位，（那必须的，浮点数的作用之一就是确保除法不舍位）
...
*注意！*：
在读取文本的时候为什么使用int进行字符的存储？  
因为要确保c变量存得下任何文本中可能出现的值  
对于int来说，**EOF** (end of file char)类型的变量是存不下的  
EOF在stdio.h头文件中被定义，他的大小并不重要，使用EOF的原因是为了确保程序不依赖任何特定的数值  

此外，整数类型变量可以和字符常量混合运算
在统计数字出现个数的程序中，使用以下语句进行统计
`a[c - '0']++;`

**C语言中赋值可以作为更大的表达式的一部分出现**

经典写法：
`while((c=getchar()) != EOF)...`  

!=不等于的运算符优先级比赋值=运算符的优先级要高，所以必须加括号
...
在告诉CPU与低速外部设备之间进行读写缓冲，提高CPU的工作效率
...
简单讲，就是数据被写到buffer边界之外的时候，缓冲区溢出随即发生

缓冲区溢出的危害：可能导致关键内存区域被污染
...
通常将定义在参数列表中的变量称作**形式参数**  
将在参数调用中和形式参数对应的值称作**实际参数**  
...
**在所有的C函数中，都使用值传递**  
传递给被调用函数的参数值被存放在临时变量中  
所以被调用函数无法修改主调函数的参数，只能修改私有的临时副本的值  
在被调函数中，参数可以被看作便于初始化的局部变量  
指针和数组指针例外

### 自动变量 auto

自动变量是局部变量的别称，因为在函数被调用时自动被创建，在返回时自动消失

### 外部变量声明 extern

`int max;`
`{ ... extern int max; ...}`
在函数体内部显式声明外部变量，这样做的好处是，可以让外部变量的定义语句出现在函数体之后  
也就是说先声明并使用，再分配内存  
同时，因为预处理机制，这么做还可以让a变量在A文件中定义，在B文件C文件中使用，此时不得不使用`extern`关键字
话虽如此，尽量还是将全局变量定义声明在源文件的开始处
