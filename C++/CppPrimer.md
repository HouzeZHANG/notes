...
- [第一部分 C++基础](#第一部分-c基础)
  - [第二章 变量和基本类型](#第二章-变量和基本类型)
    ...
    - [2.2 变量](#22-变量)
      ...
      - [2.2.3 标识符identifier与命名规则](#223-标识符identifier与命名规则)
      - [2.2.4 名字的作用域scope](#224-名字的作用域scope)
    - [2.3 复合类型](#23-复合类型)
      - [2.3.1 引用](#231-引用)
      - [2.3.2 指针](#232-指针)
...
$$类型说明符（type\_specifier）+变量名列表$$

$$基本数据类型（base\_type） + 声明符列表（declarator）$$

>一般来说声明符号是一个变量，但事实上**还可以有更为复杂的声明符号**（参见[引用](#23-复合类型)）
...
1. separate compilation 分离式编译
2. statically type 静态类型
3. type checking 类型检查

>**C++支持分离式编译（separate compilation），源文件可以独立编译**

文件的代码需要使用其他文件的内容
比如`cin`，`cout`就调用了标准库

为了支持分离式编译：C++**将定义和声明区分开**

1. **declaration** 声明：使得名字为程序所知，一个文件想使用别处定义的名字必须包含对那个名字的声明。
2. **definition** 定义：负责创建与名字相关联的实体——除了创建名字之外，还**申请存储空间**，还可以**给其赋予初始值**
...
extern int x = 7;
```
>函数体内部这样写则编译不通过，任何变量都只能被定义一次，但是可以被多次声明

#### 2.2.3 标识符identifier与命名规则

1. 标识符由，字母数字下划线组成
2. 必须由字母或下划线开头
3. 大小写敏感
4. 长度不限
5. 不能连续出现两个下划线，也不能以下划线紧接着大写字母开头
6. 函数体外的标识符不能以下划线开头
...
3. block scope 块作用域
4. inner scope 内层作用域（**允许在内层作用域内重新定义外层作用域中的名字，内存作用域也能访问外层作用域中已有的名字**）
5. outer scope 外层作用域

不论是在程序的什么位置，使用到的每个名字都会指向一个特定的实体：变量、函数、类型等

花括号是用来划分作用域的，其是程序的一部分

>名字的作用域始于**声明语句**，结束于所在作用域的末端（精准概括）

定义在函数体之外的名字拥有**全局作用域**，程序各处都可以使用

```C++
std::cout<<::reset<<std::endl;
```
>使用作用域运算符访问全局作用域，因为**全局作用域没有名字**，所以直接这么写会读取全局作用域中的reset变量
...
1. rvalue reference 右值引用（C++11新特性）
2. lvalue reference 左值引用

>一般讲引用，是指左值引用，引用类型引用另外一种类型
...
通过将声明符写成`%d`的形式，就可以创建引用，其中`d`为变量名
`int &x=y;`

>一般情况下，我们在定义的时候初始化变量，会将初始化值拷贝到变量中，但**引用被定义的时候，直接将引用变量和被引用变量绑定（bind）到一起，因为无法令引用变量绑定到另一个变量上，所以引用变量必须在定义的时候初始化**

1. 记住，引用即别名
2. 此外，无法创建引用的引用，因为引用本身不是对象，也无法创建表达式或是字面值的引用
3. **引用的类型**在绝大多数时候必须和**被引用的对象类型**一致

#### 2.3.2 指针

1. 指针本身是一个对象，其一样也实现了对其他对象的间接访问——`movq (%rax), %rbx`
2. 指针无需在被定义的时候赋初始值（这点和引用不同）
3. 因为引用不是对象，其没有地址，所以无法定义指向引用的指针
4. 如果指针指向了其他类型的对象，对该对象的操作将发生错误

指针的值属于下列四种情况之一：
1. 指向一个对象
2. 指向该对象的下一个位置
3. 空指针
4. 无效指针（上述三种情况之外的情况）——使用无效指针会导致程序崩溃

>**objdump的一些观察**：一般编译器会直接分配**0x0**这个地址作为空指针，所以，对空指针进行间接引用，或是给空指针指向的对象赋值，都是危险的。在必要时，在使用指针之前，需要先检查指针是否为空指针
...
//等价于
int *ptr = 0;
//也等价于preprocessor variable NULL
#include cstdlib
int *ptr = NULL;
```

>`nullptr`是C++11新引入的字面量，其可以被转换成其他指针类型

>诸如`NULL`的预处理变量不属于任何命名空间，其由预处理器负责——预处理器只是简单地插入代码，使用`NULL`和使用`0`的效果是一样的
